...
func TestNewStartsOnHome(t *testing.T) {
	m := New(nil, nil, nil, domain.QualityHighest, "never", false, "catppuccin-mocha", true, 100, "1.0.0", t.TempDir())
	if m.view != viewHome {
...
func TestNewRestoresVolume(t *testing.T) {
	m := New(nil, nil, nil, domain.QualityHighest, "never", false, "catppuccin-mocha", true, 42, "1.0.0", t.TempDir())
	if m.volume != 42 {
		t.Fatalf("New should restore the saved volume, got %d", m.volume)
	}
	// Out-of-range values from a hand-edited config are clamped.
	m = New(nil, nil, nil, domain.QualityHighest, "never", false, "catppuccin-mocha", true, 150, "1.0.0", t.TempDir())
	if m.volume != 100 {
